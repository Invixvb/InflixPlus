...
public class WatchListLoader : MonoBehaviour
{
    [Header("Top Bar")]
    ...
    public void OnEnable()
    {
        ...
        // Loading all ButtonItems who have got addedToWatchList enabled
        for (var i = 0; i < StaticConfig.PublicConfig.currentUser.collectionsList.Count; i++)
        {
            ...
            if (collection.addedToWatchList)
            {
                ...
                foreach (var child in childrenObjectList)
                {
                    ...
                    {
                        case "ItemText":
                            var currentText = StaticConfig.PublicConfig.currentUser.collectionsList[i].title;
                            child.GetComponent<TextMeshProUGUI>().text = currentText;
                            break;
                        case "ClippingImage":
                            var currentImage = StaticConfig.PublicConfig.currentUser.collectionsList[i].mediaItem
                                .photoItemsList[0].texture;
